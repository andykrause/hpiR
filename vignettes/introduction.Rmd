...
<!--    hed_series <- calcIndexSeries(hpi_obj = hed_hpi, -->
<!--                                  train_period = 24, -->
<!--                                  max_period = 30) -->

<!--    # Create a series of full length, add to 'hpi' object, give it a name -->
<!--    rs_hpi <- calcIndexSeries(hpi_obj = rs_hpi, -->
<!--                            train_period = 24, -->
<!--                            max_period = 48, -->
...
<!--    # Check name -->
<!--    names(rs_hpi$series_24_to_48[1]) -->
...
<!--   rs_rev <- calcRevision(series_obj = rs_hpi$series_24_to_48) -->
...
<!--   hed_rev <- calcRevision(series_obj = rs_hpi, -->
<!--                           series_name = 'series_24_to_48') # an hpi object -->
<!--   class(hed_rev) -->

<!--   # Add to an hpi object -->
<!--   rs_hpi <- calcRevision(series_obj = rs_hpi, -->
...
<!-- The `calcAccuracy()` function requires a full `hpi` object as its main argument.  `test_type` defines the manner of testing accuracy; at the moment the only working options is `rs` which uses repeat transactions to judge the accuracy of an index.  (Additional types to come).  `test_method` sets the manner in which the test is employed, **insample**, **kfold**, or **forecast**.  If the `hpi_obj` does not match the `test_type` (in all cases `rs` for now), then you must supply the `rs` sales data in the `index_data` argument.   -->

<!-- ```{r eval=TRUE, messages=FALSE, echo=TRUE} -->

<!--     rs_is_error <- calcAccuracy(hpi_obj = rs_hpi, -->
<!--                                 test_type = 'rs', -->
<!--                                 test_method = 'insample') -->

<!-- ``` -->

<!-- The `calcAccuracy()` function returns a data.frame with the actual (repeat) sale price compared to the one predicted by the index.  The returned object is of class 'indexerrors' and 'data.frame'.   -->
...
<!-- *Note that the prediction data is trimmed due to the fact that the  -->
...
<!--   rs_kf_error <- calcAccuracy(hpi_obj = rs_hpi, -->
<!--                               test_type = 'rs', -->
<!--                               test_method = 'kfold') -->
...
<!--   rs_fc_error <- calcAccuracy(hpi_obj = rs_hpi, -->
<!--                               test_type = 'rs', -->
...
<!--   error_df <- data.frame(rsi = round(as.numeric(summary(abs(rs_is_error$pred_error))), 3), -->
<!--                          rsk = round(as.numeric(summary(abs(rs_kf_error$pred_error))), 3), -->
<!--                          rsf = round(as.numeric(summary(abs(rs_fc_error$pred_error))), 3), -->
...
<!--   names(error_df) <- c('RS - In Sample', 'RS - KFold', 'RS - Forecast',  -->
...
<!--   blend_index <- blendIndexes(index_list = list(rs_hpi$index, -->
<!--                                                 hed_hpi$index), -->
<!--                               weights=c(.6, .4)) -->
