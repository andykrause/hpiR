...
For the sake of the remainder of this analysis, let's allow the defaults to set the time boundaries based on the data itself (we)

We now have a dataset that includes three new fields: 

1. **date_period** - the relative time period of the sale based on the periodicity argument
2. **date_value** - the time period in absolute numeric terms
3. **date_name** - the time period name in string format
