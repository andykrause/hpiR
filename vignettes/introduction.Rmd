...
With the wrapper you can send it:

1. A raw sales object (sales_df = 'data.frame' object)
...
2. A salesdf object (sales_df = 'salesdf' object)
   * date (required)
   * price (required)
   * sale_id (required)
   * prop_id (required)
   ...
   * trim_model (opt: FALSE)
   * max_period (opt: max of data)
3. An 'rs' object (sales_df = 'rs' object)
   * estimator (opt: default = 'base')
   * log_dep (opt: TRUE)
   * trim_model (opt: FALSE)
   * max_period (opt: max of data)
