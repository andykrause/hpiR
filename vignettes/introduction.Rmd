...
A wrapper function, `calcSeriesAccuracy()` will calculate accuracy for all **hpiindex** objects in a series and return to an object of class `seriesaccuracy`.  
...
The calculations can occur "in place" and a series object will be returned by the function.  Note that running this function on kfold method may be computationally intense depending on your data size and index length.  
...
We can plot the `seriesaccuracy` objects, which look identical to `hpiaccuracy` objects. 
