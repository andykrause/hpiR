...
The `dateToPeriod()` function will convert R date fields into relative time periods based on a periodicity of choice.  The standard available periodicity options are:
...
**Note**: As of version 0.3.3, two additional approaches to periodicity are available:

* *"equalfreq"*: Dividing the total time period into equal periods of any given frequency (in days)
* *"equalsample"*: Dividing the data up into periods of varying lengths but of equal observations size; equal sample within each period. 

More on these two new options are discussed at the end of this section. 
...
Additional information on the full extent of the periods as well as their period number and name can be found in the `"period_table"` attribute of the `sales_hdf` object or accessed by:

```{r period_table_attr}
  knitr::kable(head(attr(sales_hdf, 'period_table')))
```
...
#### Equal Freq and Equal Sample

As noted above, as of version 0.3.3, two new periodicity options are available.  

The first is **"equalfreq"**.  Equal frequency periodicity divides the data up into period of equal frequency, regardless of calendar dates.  To use this option you must set a **'freq'** parameter to the number of days in the desired periods. In the likely case that your time period does not evenly divide up into the frequency selected, the extra days will added to the 'end'.  You can select whether this 'end' is the earliest or latest period by choosing where to start the dividing of time into period with the **'start'** parameter. 'First' ('last') will start the counting at the earliest (latest) period, resulting in the extra time being added to the last (first) period. 

```{r equalfreq_ex}
  ef_hdf <- dateToPeriod(trans_df = ex_sales,
                         date = 'sale_date',
                         periodicity = 'equalfreq',
                         freq = 45,
                         start = 'first')
   knitr::kable(head(attr(ef_hdf, 'period_table')))
```

The second option is to use equal sample periods.  In this case, the data are divided up such that each period has approximately the same number of observations.  In most cases, the periods length, in days, will vary.  To set the number of periods, provide the count to the **'nbr_periods'** parameter.  

```{r equalsample_ex}
  es_hdf <- dateToPeriod(trans_df = ex_sales,
                         date = 'sale_date',
                         periodicity = 'equalsample',
                         nbr_periods = 50)
   knitr::kable(head(attr(es_hdf, 'period_table')))
```
