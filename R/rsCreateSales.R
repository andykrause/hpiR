...
#' @param time default=NULL, field containing the time of the sale.
...
#' If passing a raw data.frame (not a 'sales.df' object) the "time" field should refer to
...
rsCreateSales <- function(sales_df,
  ...
  ## Calculate the necessary time field
  ...
  if(class(sales_df)[1] != 'sales.df'){
    ...
    sales_df <- dateToPeriod(sales_df=sales_df, date=date, period=periodicity)
  ...
  sales_df <- sales_df %>%
    ...
    dplyr::arrange(prop_id, time_period, desc(price)) %>%

    # Remove any properties sold twice in same time period
    dplyr::filter(!duplicated(paste0(prop_id, '_', time_period)))
  ...
  if(nrow(rs2) > 0){
    ...
    x_df <- sales_df %>%
      ...
      dplyr::arrange(prop_id, time_period)
    ...
    d2 <- data.frame(prop_id=x_df$prop_id[id_1],
                     time_1=x_df$time_period[id_1],
                     time_2=x_df$time_period[id_2],
  ...
  if(nrow(rs3) > 0){
    ...
    d3 <- x_df %>%
      ...
      # Add time and price
      dplyr::mutate(time_1 = x_df$time_period[match(sale_id1, x_df$sale_id)]) %>%
      dplyr::mutate(time_2 = x_df$time_period[match(sale_id2, x_df$sale_id)]) %>%
