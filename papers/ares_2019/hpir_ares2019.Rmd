...
Data used in the examples and the comparisons are from the `hpiR` open source R package. In the comparative analyses section I provide additional details on the functionality and lexicon of the `hpiR` package to assist in the reproducibility of this work.
...
### Machine Learning and Real Estate Markets

* Not unheard of, especially in industry

-- See HouseCanary, GeoPhy AVM Whitepapers
-- Zillow zestimate explanation

* Cite few examples of use of random forests in RE lit (Bourassa)
...
Conceptually, an individual conditional expectation (ICE) plot takes a single observations, X_i, and for one of the features or variables, Xs, simulates the predicted value of that observation under the hypothetical condition that this observation has each individual unique value of Xs in the entire dataset. As such, an ICE shows the marginal effect on the predicted outcome from a single feature (Friedman 2001) for a single observations or instance. Calculating ICEs for all data in the training set and averaging the marginal effects creates a partial dependence plot.  

* Insert ICE/PDP formulas

Another way to think about an ICE is to take a single property and predict what it would have sold for each month over a given time frame.  This time-series of predicted values can then be converted to an index.  Do this for all homes in your observation set to create a PDP.
