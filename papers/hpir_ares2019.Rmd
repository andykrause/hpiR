---
title: "A machine learning approach to House Price Indexes with the hpiR package"
author: 
 Andy Krause
 -- Zillow Group
 -- Seattle, WA
...
Real estate price indexes are widely used across many different property types, use cases and research designs. This paper is divided into three parts.  In the first, I provide an overview of the recently released open source package, **hpiR**,  (R language) for calculating house price indexes.  Along with this, I cover the development of a consistent terminology and concepts to discuss and compare indexes.  In part two, I introduce a new method or approach for estimating a home price index: Random Forest simulations.  Finally, in the third section I highlight hte functionality of the **hpiR** package by comparing the performance of the Random Forest-generated indexes to more traditional Repeat Sales- and Hedonic Price Model-derived indexes.
...
This initial section covers the key concepts and functionality of the `hpiR` package (v0.3.0), including discussion of terminology and included datasets.  For more in-depth information on the development of the package or to examine the history of code changes please see the additional resources below: 

* More information on this package's development can be found at [www.github.com/andykrause/hpiR](https://www.github.com/andykrause/hpiR).

* The official CRAN repository page is located at: 
[https://cran.r-project.org/web/packages/hpiR/index.html](https://cran.r-project.org/web/packages/hpiR/index.html)

* A detailed explanation of the class and method structure employed in this package can be found in the **Classes in hpiR** vignette in this package, or at: [https://cran.r-project.org/web/packages/hpiR/vignettes/classstructure.html](https://cran.r-project.org/web/packages/hpiR/vignettes/classstructure.html)
...
The syntax and specifics of each of these processes is presented in the third section of this paper.  
...
Once you have have installed it, load it like you would any other package: `library(hpiR)`
...
## Random Forest House Price Indexes

Random forests are a common modeling approach in the machine learning literature and industry (... insert more here).  However, they have been little used in academic real estate studies (add examples if any) and not at all in house price index creation. Much of the reason for this dearth is that random forests are somewhat of a black box in that they do not directly create coefficient estimates as more traditional statistical models do and, therefore, do not offer a direct approach to create price indexes.  

With the use of simulations, however, price trends can be extracted from random forest models as used to create an index.  In general, the process works like this:

1. Estimate a random forest model (including a time period variable in the specification)
2. Create a hypothetical observation with transaction dates at every available period in your model 
3. Predict the value of these observation at each period using the random forest
4. Convert this series of price predictions to a index
5. Repeat steps 2 through 4 multiple times and average the results

Note that the simulation approach is related to the creation of partial dependency plots, however it differ that the PDP average over all observations assuming each has a feature value across the range of the tested feature while the simulation approach

```{r}
  readRDS('c:/code/research/hpir/papers/ex1plot.rds')
```

```{r}
   readRDS('c:/code/research/hpir/papers/ex15plot.rds')
```

```{r}
   readRDS('c:/code/research/hpir/papers/ex15aplot.rds')
```
...
```{r}
 geo_df <- readRDS('c:/code/research/hpir/papers/geo_summ.rds')
 names(geo_df) <- c('Approach', "Volatility", 'Revision', 'Accuracy (In Sample)', 
                     'Accuracy (kFold)', 'Accuracy (Prediction t+1)')
 geo_df <- geo_df[c(1,3,5), ]
...
```{r}
 time_df <- readRDS('c:/code/research/hpir/papers/time_summ.rds')
 names(time_df) <- c('Approach', "Volatility", 'Revision', 'Accuracy (In Sample)', 
                     'Accuracy (kFold)', 'Accuracy (Prediction t+1)')
 time_df <- time_df[c(1,3,5), ]
...
<!-- ## Part 2: Random Forest Method and its Relative Performance -->
